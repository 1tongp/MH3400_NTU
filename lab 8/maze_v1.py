...
def dfs_paths(graph, start, goal, path):
    ...
    for next_node in graph[start] - set(path):
        dfs_paths(graph, next_node, goal, path + [next_node])
...
def DFS_search(g, start_node, end_node):
    ...
    # find a possible path which can reach the end node from the start node
    path = []
    result = dfs_paths(graph, start_node, end_node, path)
    print("result", result)
    print("path2", path)
    if not result:
        return []
